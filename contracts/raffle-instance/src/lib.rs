...
impl Contract {
    ...
    pub fn buy_tickets(env: Env, buyer: Address, quantity: u32) -> Result<u32, Error> {
        // Read current persisted raffle snapshot and basic checks
        let mut raffle = read_raffle(&env)?;
        buyer.require_auth();
        require_not_paused(&env)?;
        ...
        // Snapshot values we'll validate again just before committing to detect concurrent updates
        ...
        // Reserve ticket ids from the instance NextTicketId counter (but don't persist yet)
        let mut ticket_ids = Vec::new(&env);
        let current_next: u32 = env.storage().instance().get(&DataKey::NextTicketId).unwrap_or(0u32);
        ...
        // Before committing, re-read persisted values to detect concurrent state changes.
        ...
        // If state changed since our snapshot, abort so caller can retry.
        if persisted_sold != snapshot_sold || persisted_count != current_count {
            return Err(Error::InvalidStateTransition);
        }

        // Final availability check
        if persisted_sold + quantity > raffle.max_tickets {
            return Err(Error::TicketsSoldOut);
        }

        // Commit: persist tickets, update NextTicketId, TicketCount, and raffle state
        for i in 0..quantity {
            let ticket_id = first_id + i;
        ...
        // Advance the NextTicketId atomically
        let new_next = current_next + quantity;
        env.storage().instance().set(&DataKey::NextTicketId, &new_next);
